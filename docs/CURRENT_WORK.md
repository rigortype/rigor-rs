...
- **ACTIVE: compat next stage** — [plan](notes/20260718-compat-next-stage-plan.md);
  ...
  CLOSED by measurement (M1: 0 added diags on ~17k); M2-GO slices 1–4 built
  (UM 179→155, 0 FP). NEXT NEEDS A DESIGN CALL: slice-4 witnessing vs the
  ADR-0033 provenance gate, slice 5 (namespaced singletons) vs the ADR-0023
  ConstantRead gate — see the batch note; then Phase 3 new-rule surfaces.
...
- **2026-07-18 M2-GO receiver-typing batch, slices 1–4** (branch `m2-receiver-typing-batch`) — freeze-unwrap + Kernel#Array + rand + singleton RBS returns; gitlab UM 179→155, mastodon models 5→3, 0 FP; fixture 67 pins the batch; slice-4 witness payoff + slice 5 pend TWO design decisions (ADR-0033 / ADR-0023 gates). [note](notes/20260718-m2-receiver-typing-batch.md).
