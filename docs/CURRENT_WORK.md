...
- **2026-08-25 the frozen-index ARC (Rust Glancer recon → 4 slices, COMPLETE)** — Glancer validates our standing choices (no Salsa, flat arenas, save-driven invalidation); everything portable converged on the harvest/merge split of `SourceIndex`. Landed: **#92** keystone (PR #95: stage 2 −29%, bit-identical; pre-#92 path kept as a `cfg(test)` oracle); **#94** per-candidate ancestor closure (PR #98: stage 2 @ 4,675 164.9→82.5ms); **LSP held-harvest** (PR #99: dispatch 112.8→69.8ms @ 4,675 — **the OverlayGuard stays ON; cross-file diagnostics live at gitlab scale**); #96 fp_audit determinism (PR #97 + the null-rule root-fix `31ac9ce`). Probe corrections stand: file order is NORMATIVE (merge never sorts), AST eviction stays blocked (M3 consumes ASTs). #93 diag cache NO-GO; **#104 harvest cache NO-GO** — the split `RIGOR_TIMING` stage-1 label prices it at **4.1–4.7ms of wall @ 4,675 (1.2%)**, under the ~48ms its cache's entry-read sweep costs — [s104](notes/20260826-s104-harvest-cost-measured.md). Cross-file hover/completion LANDED (PR #101: per-URI last-good cache — [impl](notes/20260826-lsp-crossfile-cache-impl.md)); #102 CLOSED (PR #103: `FileKey` path identity). [recon](notes/20260825-rust-glancer-frozen-index-recon.md) / [inventory](notes/20260825-s92-buildproject-pass-inventory.md) / [s92](notes/20260825-s92-harvest-merge-impl.md) / [s94](notes/20260825-s94-ancestor-closure-impl.md) / [lsp](notes/20260825-lsp-held-harvest-impl.md).
