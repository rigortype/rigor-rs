...
- **2026-09-21 #128 CLOSED — store values named by their TYPED answer** (PR #131) — the SYNTACTIC classifier both lost rows and left latent FPs the sweep could not reach; the store's member is now the erased class of the value's `stmt_value_type` answer off the pass's sparse env. **0 FP / 9,337, matched counts = master**; fixture 108 +r19–r22. [note](notes/20260921-issue-128-store-value-typing.md).
