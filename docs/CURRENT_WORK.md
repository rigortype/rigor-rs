...
- **2026-08-08 narrowing carrier fidelity — the subset argument was FALSE, 19 live FP carriers** (PRs #71, #72) — every narrowing spec rested on "we narrow only `Dynamic`/`Top`, so we are a strict subset". That holds only if both engines agree WHICH locals are Dynamic, and they do not: we collapse `a || b`, `(1..2)`, `proc {}`, `__method__`, `defined?`, `begin/rescue`, a loop-as-value and 12 more to `Dynamic[top]` where the reference keeps a union or a real type. 92 oracle probes; fixed by an ALLOW-list (a deny-list leaks — `__method__` and `proc {}` are arena-identical to safe receiverless calls). Cost 3 rows, 0 FP / 9204. Stage 3b-1 rode along (**6 closures**, both census archetypes). A SECOND family is still open: a disjoint guard (`h = [1,2]` then `is_a?(Hash)`) is a dead branch the reference silences and we do not. [note](notes/20260808-narrowing-carrier-fidelity-fp.md).
...
- **2026-08-07/08 class-narrowing arc, stages 1–2 + the position rule** (PRs #63, #68) — snapshot-pass port of `narrow_class_other` (Dynamic→`Nominal[C]`, truthy edge only) + a `Node::When` arena split; **11 closures**, 3 unprobed edges declined on review, and the census's `to_s` slice REFUTED by probe. #63's safe-nav decline then proved the WRONG AXIS: block bodies and `case` clauses narrow only in STATEMENT position or a local-write RHS (`=`, `a,b=`, `||=`) — receiver, argument and `return`-operand decline, while `if`/ternary narrows everywhere. TEN live FP shapes fixed; `stmt_position` threads the whole pass because an expression-position conditional must carry the drop INTO its branch bodies. **0 FP / 9204** at every step, reviewer-verified row for row. [position rule](notes/20260807-block-narrowing-position-rule.md) / [spec](notes/20260807-class-narrowing-slice-spec.md).
...
- **2026-07-31 sig-gen reads `Data.define` / `Struct.new` classes** (upstream #227, past the pin) — an empty class SHELL is worse than none: declaring the class narrows dispatch to a nominal and the inherited `::Data.new: () -> bot` makes every construction an arity error. Both spellings now emit member readers (+ Struct writers), a `.new`/`.[]` pair, and the `::Data`/`::Struct[untyped]` ancestry. **Byte-identical to upstream `master`**; corpus match 20→30 files, diagnostic gates UNMOVED. [note](notes/20260731-siggen-data-struct-members.md).
- **2026-07-31 `BigMath` ingestion asymmetry CLOSED — the oracle is BLINDED, not ignorant** — the reference LOADS `module BigMath`, its DEFINITION build raises and fails soft to nil, so it is silent on every method while `class_known?` stays true. Mirrored by `UNBUILDABLE_DEFINITIONS` (12 classes), regenerable via `harness/unbuildable_classes.rb --check`. **The set is keyed to pin × installed gems** — measured A/B. Output-neutral. [note](notes/20260731-bigmath-ingestion-asymmetry.md).
...
- **2026-07-18 compat arc (Phases 0/1/3) + three subsystems, folded** — Phase 0 characterisation ([findings](notes/20260718-phase0-m1-m2-findings.md)); Phase 1 fixture parity 100% (PR #24); Phase 3 unknown-config-key warning byte-exact ([note](notes/20260718-phase3-new-rule-surfaces.md)); the M2-GO receiver-typing batch, gitlab UM 179→148 ([note](notes/20260718-m2-receiver-typing-batch.md)); severity machinery ([note](notes/20260718-severity-profile-machinery.md)); `--bleeding-edge` ([note](notes/20260718-bleeding-edge-void-rule.md)); the rbs-inline auto-wire measurement that HELD the pin ([note](notes/20260718-upstream-rbs-inline-autowire-regressions.md)).
