...
- **2026-08-08 class narrowing survives by POSITION, not safe-nav — TEN live FP shapes fixed** (PR #68) — PR #63's safe-nav decline was the wrong axis: it suppressed shapes the reference FIRES on and suppressed none of the FPs. Measured rule: block bodies and `case` clauses narrow only in STATEMENT position or a local-write RHS (`=`, `a,b=`, `||=`); receiver, argument and `return`-operand positions decline — while `if`/ternary narrows in every position. `stmt_position` now threads the whole pass, because an expression-position conditional must carry the drop INTO its branch bodies. **1168→1167, 0 FP / 9204 files** (reviewer-verified on the hardened harness; 25-row both-engines matrix agrees row for row). [note](notes/20260807-block-narrowing-position-rule.md).
