...
- **2026-07-31 `BigMath` ingestion asymmetry CLOSED — the oracle is BLINDED, not ignorant** — the s2 note's cause was wrong — the reference DOES load `module BigMath`; the DEFINITION build fails. `DEFAULT_LIBRARIES` lists `bigdecimal` AND `bigdecimal-math`, rbs prefers an installed GEM's `sig/` over its own `stdlib/` copy, and `bigdecimal-4.1.2/sig` ships `big_math.rbs` too ⇒ two decls ⇒ `DuplicatedMethodDefinitionError` ⇒ definitions fail-soft to nil ⇒ every call degrades to `Dynamic[Top]`. rigor-rs vendors ONE copy, builds, witnessed — an ADR-0002 FP. Matched it: `UNBUILDABLE_DEFINITIONS` keeps the class KNOWN (else the FP becomes an unresolved-toplevel one) and empties its METHOD surface, instance/singleton tracked apart. A TABLE not a derivation — the colliding decl is exactly the one rigor-rs lacks; regenerated by `harness/unbuildable_classes.rb --check` (pin-bump ritual). **12 unbuildable classes, 2 observably divergent**; self-diff **0/0 / 9204 files** — no real file writes the shape, so the sweep structurally cannot see this class of divergence; fixture 78 is the only guard. [note](notes/20260731-bigmath-ingestion-asymmetry.md).
