...
▶ **NEXT (measured, 2026-08-08):** adjudicate the `Object` bucket (26) before
acting on it. Off the gap track after that: LSP v4+ or the next upstream tag.
The three 08-08 arcs are DONE: narrowing (3a-2 DEFERRED at 0 verified rows,
3a-4/3b-2 ≤1, [remeasure](notes/20260808-narrowing-3a23-window-remeasure.md)),
qualified witnessing, constant harvesting (slice C DECLINED — architectural,
...
- **2026-07-31 LSP v4 completion slice** — `Foo::` offers NESTED CONSTANTS via the ADR-0042 qualified registry, case-split after `::`. Private RBS declarations dropped from explicit-receiver completion; DIAGNOSTIC predicates still see them (witnessing absence would be an FP). [note](notes/20260731-lsp-v4-const-completion-visibility.md).
...
- **2026-07-31 standing FP sweep set CODIFIED** — `harness/sweep-corpora.yml` is the single membership list for BOTH sweep tools; an absent corpus reports SKIPPED. Also fixed `run_corpus.rb`'s `REFERENCE_RIGOR_DIR` default (pointed at the WORKING checkout — hazard 3 baked into a gate). Baseline 0 FP / 9204 / 8 corpora ([CORPUS.md](../harness/CORPUS.md)).
