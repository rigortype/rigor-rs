...
- **2026-08-07 `is_a?`/`case-when` class narrowing — PR #63 ready** — snapshot-pass port of `narrow_class_other` + `Node::When` arena split; **11 closures (1193→1182), 0 FP / 9204**; 3 unprobed edges declined on review; reference inconsistent on safe-nav blocks; `to_s` slice REFUTED. [spec+outcome](notes/20260807-class-narrowing-slice-spec.md).
- **2026-08-07 upstream feedback batch 2 COMPILED** — the 3 reference-side defects found since the 07-16 batch, paste-ready repros: the `c7f28da1` (#271) master FP, the `Dynamic|nil` possible-nil FP class, the fail-soft definition build blinding 12 classes. [note](notes/20260807-upstream-feedback-batch2.md).
