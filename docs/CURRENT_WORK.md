...
asymmetry (**BOTH halves closed**). ▶ NEXT: LSP config reload (`.rigor.yml` is
read once at startup); or LSP v4+ (`rootUri`, UTF-16 incremental sync). Sweep
...
- **2026-07-31 `BigMath` ingestion asymmetry CLOSED — the oracle is BLINDED, not ignorant** — the s2 note's cause was wrong — the reference LOADS `module BigMath`; the DEFINITION build fails. `DEFAULT_LIBRARIES` lists `bigdecimal` AND `bigdecimal-math`, rbs prefers an installed GEM's `sig/` over its own `stdlib/` copy, and `bigdecimal-4.1.2/sig` ships `big_math.rbs` too ⇒ two decls ⇒ `DuplicatedMethodDefinitionError` ⇒ definitions fail-soft to nil ⇒ every call degrades to `Dynamic[Top]`. rigor-rs vendors ONE copy, builds, witnessed — an ADR-0002 FP. `UNBUILDABLE_DEFINITIONS` keeps the class KNOWN (else the FP becomes an unresolved-toplevel one) and empties its METHOD surface, instance/singleton apart. **Keyed to pin × INSTALLED GEMS, not the pin alone** — A/B measured: drop the `bigdecimal` gem and the same pinned oracle FIRES on both shapes, set 12→11 (`BigMath` is the ONLY env-supplied entry). So run `harness/unbuildable_classes.rb --check` WHERE THE GATES RUN; it tags sources `[env]`/`[pin]` so a diff reads as "a gem moved". **12 unbuildable classes, 2 observably divergent**; self-diff **0/0 / 9204 files** — no real file writes the shape, so the sweep cannot see it; fixture 78 is the only guard. [note](notes/20260731-bigmath-ingestion-asymmetry.md).
...
- **2026-07-19 upstream tracking `b70adcb5..ff6b6158`** (29 commits, three waves: transitive-void ADR-100 WD4 / type-of plugin-env parity; the #194 loader stack landed+closed upstream, doctor skew check; ADR-58 massign-ivar seeding, ADR-67 WD6 call-site param inference) — hardened self-diff **0 added / 0 dropped** on all four battery corpora every wave; nothing to port (substrate-blocked large arcs, precision-additive, or plugin-loader-only); pin `7a69f142` held throughout, no v0.3.0 tag yet. NEW oracle hazard 2 in `UPSTREAM.md`: the reference result cache is not version-scoped — pin-vs-tip self-diffs REQUIRE `--no-cache` + isolated cwds.
...
- **2026-06-26…07-06 pre-/foundation era (12 arcs, all closed — detail in the linked notes/ADRs)** — sidecar COMPLETE + its perf slices retired by measurement ([ADR-0036](adr/0036-ruby-sidecar-default-reversal.md)/[0037](adr/0037-sidecar-perf-slices-retired-by-measurement.md)); the RBS ingestion legs ([ADR-0033](adr/0033-project-sig-ingestion.md)/[0034](adr/0034-rbs-collection-ingestion.md), inline deferred [0035](adr/0035-inline-rbs-deferred.md)); `fp_audit` + 4 real FP clusters → **0 FP across ~4000 files / 20+ libs**; the productization cluster (triage/annotate/diff/config-audit/baseline area/`check <dir>`); the flow-substrate arc ([ADR-0038](adr/0038-flow-substrate-incremental-narrowing.md)/[0039](adr/0039-shape-typing-tier.md), [0041](adr/0041-project-method-nilable-return.md) deferred) with the standing finding **no cheap FP-safe flow wins left** ([note](notes/20260706-flow-frontier-exhausted.md)); rayon (~2.4×) + LSP v1/v2 + MCP + two new flow rules. Before it: the rustfmt stance ([ADR-0032](adr/0032-source-formatting-policy.md)), v0.0.1 release prep, and the leniency alignment (witnesses restricted to RBS-known core-surface receivers per the ref's tier-4 leniency, 2 FPs fixed).
