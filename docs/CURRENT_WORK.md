...
MultiWrite substrate (#46/#47), LSP `exclude:` parity (#45), the `BigMath`
ingestion asymmetry. ▶ NEXT: the `Object#Nokogiri` half of that asymmetry — the
INVERSE direction, so it needs a vendoring decision, not a mask
([note](notes/20260731-bigmath-ingestion-asymmetry.md)); LSP config reload
(`.rigor.yml` is read once at startup); or LSP v4+ (`::` completion, visibility
filter, `rootUri`). Sweep set CODIFIED (`sweep-corpora.yml` + `--sweep`).
...
State: harness **79 fixtures / 0 FP / 3 gaps / 1 registered divergence** (live + snapshot, vs pin
...
- **2026-07-19 ADR-0042 Slices 1–4** (branches `adr-0042-impl`/`-instance`) — the qualified-key substrate (additive, gates byte-unchanged) + qualified singleton witnessing, then qualified INSTANCE witnessing incl. the fixture-70 shadow-sig unsoundness fix. [ADR](adr/0042-qualified-key-index-registration.md).
...
- **2026-07-17 ATM arc (3 slices) + P2 `Regexp.last_match` nilable source** — `call.argument-type-mismatch` both channels + per-overload/per-param RBS retention + acceptance walk, byte-exact messages (msgdiff gate), 3 named typer-substrate gaps left open ([plan](notes/20260717-atm-substrate-arc-plan.md)); P2 (`6592ead`) cut gitlab lib possible-nil 169→162 but REFUTED the broad hypothesis — the ref's wide firing rides its permissive `Dynamic|nil` arm, which our substrate deliberately cannot mint ([spec](notes/20260717-p2-optional-local-nil-spec.md)). 0 FP all corpora.
- **2026-07-17 the two undefined-method receiver-typing wins, MERGED** — C1+C2+C5 constant-shadow gate (lexical ConstantRead suppression + param-default lowering + literal `CONST=` harvest; gitlab UM 356→200, the port's largest single win) then C3a String-tail (`b6d13e9`: `self.class.name`/`to_s` tail + core-Singleton `name`; UM 200→179, whose first impl's 12 FPs were caught by per-part fp_audit and narrowed the design). 0 FP both. [gate](notes/20260717-constant-shadow-gate-spec.md) / [C3a](notes/20260717-c3a-nominal-return-tail-spec.md).
