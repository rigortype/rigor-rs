...
▶ **NEXT (2026-08-26): the EFFECT-SYSTEM arc is at slice 2 (direct
summaries LIVE, PR #105)** — next is slice 3, the taint bit + `causes`
([ADR-0043](adr/0043-effect-system-port-parity-model.md) names the order);
it converts 30 of the 34 UNDERs to MATCH, since the port emits
`exhaustive: false` everywhere today. Then #104 (harvest cache, GO).
Pin HOLDS at `v0.3.4` (master surveyed: 0 diagnostics on 9204). Also open:
the harvest cache = #104 (A GO / B NO-GO,
[remeasure](notes/20260826-harvest-cache-remeasure.md)); LSP v4+. The narrowing frontier stays OUT OF CARRIER
...
- **2026-08-26 effects slice 2 — DIRECT summaries live (PR #105)** — a Prism-walking, **typer-free** collector (constant-path receivers + implicit self only, so the handled set is a strict subset of upstream's and ADR §1 is a dependency fact) + minimal `rigor effects --full --format=json`: 11 construct origins, all six narrowing handlers, mutator sets now VENDORED (`mutators.yml`, same drift gate). Differential lands EXACTLY on the probe's predicted table — **12 MATCH / 34 UNDER / 0 OVER / 0 DM**; slice 3 converts 30 of those UNDERs. **Subset argument failed a FOURTH time**, inside a spec I wrote: a catalogue ROW can make upstream NARROWER where we lose the receiver type (`Thread#[]=` proves `global.write`, not `mutate.instance`), so `[]=`-shaped selectors are suppressed on the uncatalogued path — derived from the catalogue, pinned by a tripwire. [probe](notes/20260826-effects-s2-probe.md) / [spec](notes/20260826-effects-s2-mini-spec.md) / [impl](notes/20260826-effects-s2-impl.md).
