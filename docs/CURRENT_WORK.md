...
- **2026-07-31 LSP v4 completion slice** — `Foo::` now offers NESTED CONSTANTS via the ADR-0042 qualified registry, split on the CASE of the name after `::`. Private RBS declarations are no longer offered on an explicit receiver, while the DIAGNOSTIC predicates still see them as present (`send(:foo)` dispatches ⇒ witnessing absence would be an FP). [note](notes/20260731-lsp-v4-const-completion-visibility.md).
...
- **2026-07-31 standing FP sweep set CODIFIED** — `harness/sweep-corpora.yml` is the single membership list for BOTH `fp_audit.py --sweep` and `run_corpus.rb`; an absent corpus reports SKIPPED, never drops silently. Also fixed `run_corpus.rb`'s `REFERENCE_RIGOR_DIR` default — it pointed at the WORKING checkout, hazard 3 baked into a gate. Baseline 0 FP / 9204 / 8 corpora ([CORPUS.md](../harness/CORPUS.md)).
