...
- **Pin is `v0.3.1`** (+ vendored rbs 4.1.0). Upstream master is **+150 with no
  tag** and its tip bumps rbs to 4.1.1 — a survey is in flight; the +49 survey's
  "one diagnostic" delta is superseded (`UPSTREAM.md`, all THREE hazards).
...
State: harness **80 fixtures / 0 FP / 3 gaps / 1 registered divergence** (live + snapshot, vs pin
...
- **2026-08-07 coverage-gap CENSUS — 1193 gaps are six mechanisms, not one number** — new `harness/gap_census.py` classifies every gap by MECHANISM (from the reference's own message); the per-rule histogram hides which. **576 (48%) sit behind decisions already made** (possible-nil 435 Tier B/C, always-truthy 141 flow frontier) — quoting the total invites re-litigating them. Actionable pool = `undefined-method` 446: **~110 on CORE classes** (String 52, Object 26, Hash 17) where the port HAS the signature and just did not resolve the receiver; ~77 on the corpus's own classes (49 carry the reference's `pre_eval:` hint, ADR-17 — a mechanism we lack); 63 nil-typed; 92 shape. Three evidenced slices: `is_a?`/`case-when` narrowing (54), `X.to_s` → String, inherited singleton returns. [note](notes/20260807-gap-census.md).
- **2026-08-01 `Object#Nokogiri` asymmetry was already CLOSED — and the sibling sweep found a live FP next door** — both engines silent at the pin: `800b3a1` (the 24-FP triage) had already vendored `data/vendored_gem_sigs/`, whose `nokogiri.rbs` carries the `data/` tree's ONLY `class Object` reopen. Two notes carried the stale claim forward un-remeasured; both now carry dated corrections. Non-vacuity by ABLATION: delete that block and 4 diagnostics appear (one of them a `call.unresolved-toplevel` neither note predicted). Sweeping the whole `Object#`-level conversion-function family (14 names, off the ORACLE's own built surface) found ingestion at parity for ALL 14 — but the zero-arg column exposed a `call.wrong-arity` FP on exactly the four with a required-keyword overload: **`arity_eligible?` was never ported** (no envelope when ANY overload has a required keyword / trailing positional / `UntypedFunction` — 507 of 11,115 methods). Sweep OUTPUT-NEUTRAL: **0 FP / 9204 files**, gap table identical; harness 80 / 0 FP; fixture 80 pins both mechanisms + a keyword-free positive control. [note](notes/20260801-nokogiri-ingestion-asymmetry-closed.md).
- **2026-08-01 LSP config reload — `.rigor.yml` re-parsed by every structural `invalidate`** (S4 limitation CLOSED). Decision **BEAT the reference**: it still rebuilds from the retained `@configuration` at `v0.3.1` AND on master (its one post-pin LSP commit `9594732b` adds an `IncrementalSession`, still never re-reading it) — NO diagnostic-set parity obligation here, and matching was worse than doing NOTHING: the watcher fired, the context rebuilt, and it republished a byte-identical stale answer. A **broken** file keeps the LAST GOOD config (an editor saves half-written YAML constantly; defaults are a config the user never wrote) + ONE warning on the transition; a **deleted** one reloads to defaults — the absent-vs-malformed distinction `Config::load` collapses, now `ConfigRead`. Rides the S4 generation bump, so an in-flight worker on the old config is dropped + re-dispatched; a `.rb` save keeps the cheap N3 path. 8 tests / 7 mutations; gates UNMOVED (**79 fixtures / 0 FP / 3 gaps / 1 divergence**; sweep **0 FP / 9204 files**). [note](notes/20260801-lsp-config-reload.md).
...
- **2026-07-31 LSP v4 completion slice** — `Foo::` answered with SINGLETON METHODS (one lowercase stub served both separators); it now offers the NESTED CONSTANTS via `namespace_children` over the ADR-0042 qualified registry, split on the CASE of the name typed after `::` — Ruby's own rule, the same answer the reference reaches by dispatching on the parsed node class, without its second parse. **Private RBS declarations** (per-`def` AND bare `private` section → `ClassEntry.private_methods`, under the `void_methods` reopen gate) are no longer offered on an explicit receiver: `"x".respond_to_missing?`, `Time.refine` are NoMethodErrors. The DIAGNOSTIC predicates deliberately still see them as present (`send(:foo)` dispatches ⇒ witnessing absence would be an FP) — hence the sweep is unchanged: **0 FP / 9204 files**, gaps reproducing `CORPUS.md` entry-for-entry, harness 76 / 232 matched / 0 FP. Union receivers complete the per-arm INTERSECTION. 11 tests, each re-broken. [note](notes/20260731-lsp-v4-const-completion-visibility.md).
...
- **2026-07-19 upstream tracking `b70adcb5..ff6b6158`** (29 commits, three waves: transitive-void ADR-100 WD4 / type-of plugin-env parity; the #194 loader stack landed+closed upstream, doctor skew check; ADR-58 massign-ivar seeding, ADR-67 WD6 call-site param inference) — hardened self-diff **0 added / 0 dropped** on all four battery corpora every wave; nothing to port; pin `7a69f142` held throughout. Its three apparent "upstream regressions" were **#194 root-caused**: `rigortype 0.2.4`'s pre-gate plugin copy hijacking the auto-wire require — corrected delta 0/0, oracle invocations hardened (`harness/lib.rb`, `fp_audit.py`, `UPSTREAM.md`). NEW oracle hazard 2: the reference result cache is not version-scoped — pin-vs-tip self-diffs REQUIRE `--no-cache` + isolated cwds. [note](notes/20260718-upstream-rbs-inline-autowire-regressions.md).
- **2026-07-19 `coverage` precision mode + MCP tool + over-claim audit** (PRs #33/#40, node-level, 3 review rounds) — the reference precision-tier scan ported on rayon (`--workers`), the MCP `rigor_coverage` tool, and a node-granularity audit that found 0 factually-wrong claims once histograms stopped netting over-claims against under-claims.
...
- **2026-07-19 #194 root-caused: stale-gem plugin hijack** — the 3 "upstream regressions" were artifacts of `rigortype 0.2.4`'s pre-gate plugin copy hijacking the auto-wire require; corrected wave delta 0/0; oracle invocations hardened (`harness/lib.rb`, `fp_audit.py`, `UPSTREAM.md`). [note](notes/20260718-upstream-rbs-inline-autowire-regressions.md).
