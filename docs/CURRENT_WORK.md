...
the frozen-index arc — next: the LSP held-harvest slice (#92/#94 done);
...
- **2026-08-25 the frozen-index ARC: Glancer recon → #93 baselines → #92 keystone LANDED (PR #95)** — Rust Glancer validates our standing choices (no Salsa, flat arenas, save-driven invalidation); every portable idea converges on the per-file harvest/merge decomposition of `SourceIndex`, now SHIPPED: harvest rides stage 1's rayon closure, stage 2 is merge-only (−29%), bit-identical everywhere (0 FP / 9,204 + gap set unchanged; order-leak + coupling examples pinned; the pre-#92 path kept under `cfg(test)` as the equivalence oracle). Probe corrections: merge-resident passes ≈57%, Pass 4b alone 43–47% (→ #94) — AST eviction and the OverlayGuard's fall need MORE than the keystone. #93 baselines: skippable 77–97%, cliff + 241MB held ASTs reproduced live → LSP-only GO, harvest cache GO-after-#92, diag cache NO-GO. **#94 LANDED** (PR #98): per-candidate ancestor closure — stage 2 @ 4,675 164.9→82.5ms (−50%), merge alone now fits the 100ms guard budget at the median ([probe](notes/20260825-s94-pass4b-cost-probe.md) / [spec](notes/20260825-s94-ancestor-closure-mini-spec.md) / [impl](notes/20260825-s94-ancestor-closure-impl.md)); fp_audit render determinism fixed (#96 / PR #97). [recon](notes/20260825-rust-glancer-frozen-index-recon.md) / [baselines](notes/20260825-frozen-index-baseline-measurements.md) / [inventory](notes/20260825-s92-buildproject-pass-inventory.md) / [spec](notes/20260825-s92-harvest-merge-mini-spec.md) / [impl](notes/20260825-s92-harvest-merge-impl.md).
