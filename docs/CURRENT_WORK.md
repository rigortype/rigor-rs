...
- **2026-07-31 upstream pin `7a69f142 → v0.3.0`** (tag `5802c990`, 42 commits) — behaviour-neutral in every shipped profile: snapshots byte-identical, live+snapshot gates 0 gaps / 0 FP, no source change. Sole delta on ~9200 files = **4 reference-only diagnostics**, bisected to `861b08b9` (ADR-93 auto-wire of `rigor-rbs-inline`) → [ADR-0035](adr/0035-inline-rbs-deferred.md)'s inline-RBS deferral made measurable; `parameter_inference:` / `value-use.void` stay off by default; `ARRAY_NEW_TUPLE_LIMIT` still 16. **Side finding**: 24 pre-existing FPs (identical at BOTH pins) on never-swept corpora — own arc. **`v0.3.1` gated on vendored rbs 4.0.3→4.1.0.** [note](notes/20260731-upstream-bump-7a69f142-v030.md).
