...
- **2026-08-25 Rust Glancer recon — frozen-index direction SET, keystone filed** — the memory-frugal Rust LSP validates our standing choices (no Salsa, flat arenas, save-driven invalidation); its portable ideas all converge on ONE keystone: per-file harvest/merge decomposition of `SourceIndex` (= the S4b-deferred `extend_with`), unlocking O(1-file) keystrokes, AST eviction (189MB @ 4,675 files), cross-file hover/completion, and the ADR-0017/0028 cache. Issues #92 (keystone) / #93. [note](notes/20260825-rust-glancer-frozen-index-recon.md). **#93 MEASURED**: skippable 77–97% and scale-growing (286ms @ 4,675), guard cliff + 241MB ASTs live → LSP-only GO, harvest cache GO-after-#92, diag cache NO-GO ([baselines](notes/20260825-frozen-index-baseline-measurements.md)).
