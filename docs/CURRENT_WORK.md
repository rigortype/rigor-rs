...
DECLINED (see Standing conclusions). AST eviction is gated on one unmeasured number
([probe](notes/20260826-ast-eviction-probe.md)).
...
- **The effects TRANSITIVE LABEL lane is DECLINED — not portable at parity** (2026-08-26 slice-4 probe). Four progressively stricter typer-free rules were built and MEASURED; the best is still 5 OVER on gitlab-foss/lib. Upstream's edge set IS the set of call nodes its typer visited, and those visits are not syntactically characterisable (`return` values, `unless` bodies, `elsif` arms, interpolation, `next`/`break`…). The inversion that settles it: more edges ⇒ more TAINT (sound) but more edges ⇒ more LABELS (unsound), and upstream's `absorb` moves both in ONE pass. Slice 3's over-taint stand-in stays; the labels stay UNDER, which is always safe. The ~2,000-method prize is reachable ONLY by weakening ADR-0043's OVER gate with a registered-divergence device for demonstrated upstream traversal gaps — a bar-weakening decision, not a slice. [probe](notes/20260826-effects-s4-probe.md).
