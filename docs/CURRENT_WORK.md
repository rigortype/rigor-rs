...
- **2026-09-09 #118 CLOSED — the generic dispatch stops pinning the flat slot** (PR #120) — tier 3's flat slot DROPPED the nil bit (`String#[]` arms all `String?`); declines to `Dynamic[top]` when the join is at risk AND an argument is reference-untyped: **13 FPs closed, 0 matched rows lost, corpus byte-identical**. A blanket "nilable declines" cost TEN folded-literal rows — untypedness is what stops the reference folding. Residue #121 (closed by PR #149). Fixture 103 now tests `RUBY_ENGINE`, not the host-patch-sensitive `RUBY_VERSION`. [note](notes/20260909-generic-dispatch-untyped-arg.md).
...
- **2026-08-26/28 the effects GATE was lying, three times** (PRs #112/#115/#117) — (a) a DELETED taint arm scored byte-identically, so real projects (`mastodon/app`; `gitlab-foss/lib` behind `--scale`) now run by default and the wrong arm fails by **76 OVER**; (b) the `methods: {}` guard covered 2 of **4** declared-lane producers, hidden by (a)'s config — **a normalisation that removes a confound can remove coverage**; (c) `omit?` INVERTS ADR-0043 §2: the port's taint bit can only DROP a row. All found by trying to make the gate lie. [s112](notes/20260826-s112-effects-instrument.md) / [s5](notes/20260826-effects-s5-probe.md) / [s116](notes/20260826-s116-snapshot-gate.md).
