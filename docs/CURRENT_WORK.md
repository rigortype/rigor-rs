...
the narrowing frontier stays OUT OF CARRIER LEVERS; #121 (a class-guarded
argument, needing the port's folder to learn `String#[]`) is still open.
...
- **2026-09-21 upstream re-pin `v0.3.8 → v0.3.9`** (447 commits, upstream's largest `Fixed` list; branch `upstream-pin-v0.3.9`) — **0 FP / 9,337 files, gaps 799→892**, harness 107→**108** fixtures (527 matched), effects gate 0 OVER, both exception tables and the registry still EMPTY. Only ONE re-sync half moved (plugin sig 1,735→1,840, #916's `TimeWithZone < Time`; fixture 98 gained rows for it). Raw bump = 3 fixture FPs + **1 sweep FP** = THREE families: `6cde8381` (#657, `declines_bot?` reaches the SHAPED carriers — the singleton half needs no port, probed silent on both), `152f7c9f` (#883, a rooted `::RUBY_VERSION` folds — the source-spelling re-check was also doing the DYNAMIC-BASE job, now a lowering bit), and `1ad7351e` (#580 `MutationRejoin`, sweep-only). **The F-C first cut passed the 0-FP gate while costing a matched row** (mastodon 420→419): collapsing a disagreeing join to untyped cannot re-join, and a store's value class is a SET (a ternary contributes both arms). `--sweep` is 3 minutes again (#874); `data/capability_roles/` (#976) is a new pin surface deliberately NOT vendored (no `conforms-to` support). [note](notes/20260921-repin-v039.md).
...
- **2026-09-09 upstream re-pin `v0.3.4 → v0.3.8`** (924 commits / 4 releases) — **0 FP / 9204, gaps 820→799**, harness 98→**105** fixtures, effects 0 OVER. Every re-sync half moved (rbs 4.2.0, plugin sig ×2, effects catalogue + snapshot schema 2). Raw bump = 7 fixture retractions + 12 sweep FPs = **SIX families**, each bisected and ported by four worktree agents; plus the effects gate's own retraction (#446 `super` taints). **Three spec claims were wrong and the must-fire controls caught them.** Residues: #118 (closed same day), 20 resolvable-`super` rows. [note](notes/20260909-repin-v038.md) / [spec](notes/20260909-repin-v038-port-spec.md) / [feedback 4](notes/20260909-upstream-feedback-batch4.md).
...
- **2026-08-09 unresolved-const-receiver carrier — BUILT, REJECTED at 0 rows** (PR #89, closed) — **841→841 on 9204**; its first allow-list member needs both engines' INDEXES to agree, invisible to a core+stdlib sweep. [note](notes/20260809-unresolved-const-receiver-carrier.md).
