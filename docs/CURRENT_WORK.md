...
▶ **NEXT (2026-08-26): the EFFECT-SYSTEM arc is at slice 3 (the taint bit
LIVE)** — next is slice 4, the transitive LABEL lane + the `resolved` bit
([ADR-0043](adr/0043-effect-system-port-parity-model.md) names the order); it
owns the 3 missing-label UNDERs and replaces slice 3's selector-set edge taint
with the real closure. Then LSP v4+.
...
- **2026-08-26 effects slice 3 — the TAINT bit + `causes` (+ the #106 posture over-fix, PR #107)** — `exhaustive` is upstream's **TRANSITIVE** bit; emitting the direct one is a 10 / 986-OVER failure. The port computes it typer-free by tainting at every undecidable site AND at `push_edge` when the selector names a project unit — a **selector-set stand-in for the transitive AND**, sound: `targets_for` resolves only to keys the collection holds. Lands EXACTLY on the pinned table (**35 MATCH / 11 UNDER / 0 OVER / 0 DM** on the four predicted, kind for kind); `05_posture` 61/72/0, all 53 must-still-fire controls MATCH; **mastodon/app 4,514 → 5,217 MATCH, extra-taint 945 → 242 (74% closed), 0 OVER, proven lane unmoved**. `port-incomplete` RETIRED — `causes` is upstream's closed enum and `causes.empty? == exhaustive` now holds. New gate `06_edge` (a catalogue-CLAIMED `Kernel#format` call still keeps a project edge): with the edge taint off it is the ONLY project that fails, OVER 2. Before it, #106: the posture tier reads the TYPER's `dynamic` bit, so 8 catalogued classes the oracle cannot resolve were live over-claims — tier dropped, `05_posture` generated from the vendored catalogue. [probe](notes/20260826-effects-s3-probe.md) / [spec](notes/20260826-effects-s3-mini-spec.md) / [impl](notes/20260826-effects-s3-impl.md) / [#106](notes/20260826-s106-posture-over-fix.md). Corpus now 7: `05_posture`/`07_mutators` are GENERATED from the vendored tables (#106's lesson; 07 = 0 OVER, controls variant-proven, its 72 selectors UNREACHABLE in the port — [note](notes/20260826-s110-mutator-corpus.md)).

- **2026-08-25/26 effects slices 0-2, folded** — s0 opened the port ([ADR-0043](adr/0043-effect-system-port-parity-model.md)): a summary is graded per METHOD as a **sound subset** (proven ⊆ oracle, taint ≥ oracle, declared exact) by `harness/effects_diff.py`, the first PROJECT-level instrument — it runs IN the project, so it sees `.rigor.yml` / `sig/` / plugins; the `declared:` open is **SOLVED** (the CALLER's lane; slice 6 needs both rules). s1 = `crates/rigor-effects` (PR #100): the 420-row vendored catalogue, row→universal→posture precedence, implied-ancestor `known?`, three drift layers, stale-binary guards over the path-dep CLOSURE. s2 = the typer-free Prism collector + `rigor effects` (PR #105): **12 MATCH / 34 UNDER / 0 OVER**, the predicted table; **the subset argument failed a FOURTH time** — a ROW can make upstream NARROWER where the port loses the receiver type (`Thread#[]=` is `global.write`, not `mutate.instance`), so `[]=`-shaped selectors are suppressed uncatalogued. [s0](notes/20260826-effects-s1-catalogue-probe.md) / [s1](notes/20260826-effects-s1-impl.md) / [s2 probe](notes/20260826-effects-s2-probe.md) / [s2](notes/20260826-effects-s2-impl.md).
- **2026-08-25/26 the frozen-index ARC — COMPLETE** (Rust Glancer recon → PRs #95/#97/#98/#99/#101/#103/#109) — everything portable converged on the per-file harvest/merge decomposition of `SourceIndex`: the #92 keystone (stage 2 −29%, the pre-#92 path kept as a `cfg(test)` equivalence oracle), #94's per-candidate ancestor closure (stage 2 @ 4,675 164.9→82.5ms), LSP held harvests (dispatch 112.8→69.8ms — **the OverlayGuard stays ON; cross-file diagnostics live at gitlab scale**), the per-URI cross-file cache (hover/completion leave single-file), #102's `FileKey` path identity, and #96's fp_audit determinism. **Standing**: file order is NORMATIVE in `merge` (never sort — it reaches diagnostics); AST eviction stays BLOCKED (M3 + stage 3 both need every `LoweredAst`); the harvest cache is **NO-GO** (#104: the prize is 1.2% of wall, the reads are 10× it — reopen only if the AST requirement falls). [recon](notes/20260825-rust-glancer-frozen-index-recon.md) / [s92](notes/20260825-s92-harvest-merge-impl.md) / [s94](notes/20260825-s94-ancestor-closure-impl.md) / [lsp](notes/20260826-lsp-crossfile-cache-impl.md) / [cost](notes/20260826-s104-harvest-cost-measured.md).
...
- **2026-08-01/08 instruments + adjudication, folded** — the 0-FP gate could pass VACUOUSLY (PR #65: the corpus tools measured `target/release` while `cargo build` writes debug, and `run_rs` swallowed failures into `[]`, so a panicking binary scored 0 FP; now auto-build, STALE refusal, `None`-not-`[]` — [note](notes/20260807-fp-audit-port-side-blind-spots.md)); the coverage-gap CENSUS (`gap_census.py` buckets gaps by the MECHANISM in the reference's own message — the per-rule histogram hides which; half sit behind decisions already made, so quoting the total invites re-litigating them — [note](notes/20260807-gap-census.md)); and the two 08-01 slices (`arity_eligible?` was never ported = a `call.wrong-arity` FP, fixture 80; LSP `.rigor.yml` reload on every structural invalidate, broken file keeps LAST GOOD — [nokogiri](notes/20260801-nokogiri-ingestion-asymmetry-closed.md) / [lsp](notes/20260801-lsp-config-reload.md)).
...
- **2026-08-07 upstream survey `v0.3.1`→`80aaf9bc`** (superseded by the `v0.3.2` re-pin) — 2×2 self-diff (checkout × rbs): upstream logic moves 2 diagnostics on 9204, rigor-rs silent at both; rbs 4.1.0→4.1.1 structurally 0/0. **GEM_HOME rbs selection RETIRED** (it silently dropped 1650 files); use `ruby -I <rbs>/lib -I <ext>`. [note](notes/20260807-upstream-survey-v031-to-master.md).
