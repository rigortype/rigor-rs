...
**Status (2026-06-26): fully confirmed.** With network restored, `spike/rbs_probe` adds the **`ruby-rbs` 0.3.0** crate: it builds offline-after-fetch (the native `ruby-rbs-sys` compiles via clang) and **parses the real stdlib `string.rbs` into a typed AST** — 129 method definitions extracted via its `Visit` trait, which exposes `ClassNode` (name, super → ancestors), `MethodDefinitionNode` (name/kind/visibility), `FunctionTypeNode` (`required_positionals` + return), and `TypeParam` variance. `ruby-rbs` is **parse-only** (no environment / ancestor resolution), which is exactly this ADR's design: reuse the parser, build the index ourselves. There is no "thin extraction layer" *fallback* — extracting types from the parse AST simply IS the index implementation. See [docs/notes/20260626-spike-findings.md](../notes/20260626-spike-findings.md).
