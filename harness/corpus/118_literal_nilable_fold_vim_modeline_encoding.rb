...
#
# A vim `set` modeline never parses as a `key: value` magic comment (`set`
# breaks the pair), so the fallback scan reads `fileencoding`'s `coding`
# tail followed by `=` — resolved `binary`, non-UTF-8.
#
