...
#
# The NO-SPACE form is ONE magic-comment key (`encoding=binary`), which the
# pair pass consumes — the fallback never runs and the file stays UTF-8 —
# m1 fires on both engines (probed on `ruby` itself: `"é".length` is 1).
#
