...
#  4. `unresolved:` is compared and COUNTED but never fatal. At schema 1 it
#     was the rendered cause LIST, and 4,503 of mastodon's 4,525 parameterised
#     `dynamic-receiver(…)` causes carried a reference-typer reason code
#     (`unsupported_syntax`, `inferred_return_untyped`) that a typer-free port
#     cannot produce, so grading it would be a gate that can only ever fail
#     (`docs/notes/20260826-effects-s5-probe.md` § 3). Schema 2 (`v0.3.6`,
#     upstream #434) writes HOW MANY causes instead of which — the field is an
#     integer, omitted when zero — and the port's count of typer-free causes is
#     still not the oracle's count, so the rule stands.
...
# `Snapshot::SCHEMA` (`snapshot.rb:44`) and `Snapshot::HEADER` (`:46`). Schema 2
# since `v0.3.6` (#434): `unresolved:` became a count, and an empty `reach:` table
# is preceded by `Snapshot::EMPTY_REACH_NOTE` (`snapshot.rb:371`). A schema-1
# file still loads upstream; this grader reads the PINNED reference's schema only
# and refuses any other, so a future bump that moves it fails here first.
PORT_SNAPSHOT_SCHEMA = 2
SNAPSHOT_COMMENT = ("# .rigor-effects.yml — generated by `rigor effects update`. "
                    "Commit it; review its diff.")
# `Snapshot::EMPTY_REACH_NOTE` (`snapshot.rb:371-382`), verbatim: written between
# the `methods:` and `reach:` tables whenever `reach:` is empty.
EMPTY_REACH_NOTE = (
    "",
...
def _row_wire_fields(row):
    """`Entry#to_h` (`snapshot.rb:71-76`) — the field order, and the three
    omissions a reader defaults: `declared:` when empty, `exhaustive:` when
    true, `unresolved:` when zero (schema 2: a COUNT of causes, not a list)."""
...
def render_tables(methods, reach):
    ...
    for name, table in (("methods", methods), ("reach", reach)):
        if name == "reach" and not reach:
            # `to_yaml` (`snapshot.rb:392`): the note sits between the tables.
            lines.extend(EMPTY_REACH_NOTE)
        if not table:
            lines.append(f"{name}: {{}}")
            continue
        lines.append(f"{name}:")
...
def parse_snapshot(text):
    ...
    if header["schema"] != PORT_SNAPSHOT_SCHEMA:
        raise SnapshotError(f"snapshot schema {header['schema']} is not the pinned "
                            f"{PORT_SNAPSHOT_SCHEMA} — the reference moved; re-pin this grader")
    for table in tables.values():
        for row in table.values():
            row.setdefault("effects", [])
            row.setdefault("declared", [])
            row.setdefault("exhaustive", True)
            row.setdefault("unresolved", 0)
            if not isinstance(row["unresolved"], int) or isinstance(row["unresolved"], bool):
                raise SnapshotError(f"unresolved: expected a count, got {row['unresolved']!r}")
...
def synthesise_port_methods(json_methods):
    ...
    for key, entry in json_methods.items():
        ...
        if omit(trivial=trivial, exhaustive=exhaustive, bundles=bundles,
            ...
            continue
        # Schema 2: `unresolved:` is the LENGTH of the rendered cause list
        # (`snapshot.rb:305`), never the list.
        out[key] = {"effects": proven, "declared": rendered, "exhaustive": exhaustive,
                    "unresolved": 0 if exhaustive else len(render_causes(entry.get("causes") or []))}
    return out
...
def snapshot_controls():
    ...
    """
    row = lambda eff, ex=True, un=0, de=(): {  # noqa: E731 — a table, not a function
        "effects": list(eff), "declared": list(de), "exhaustive": ex, "unresolved": int(un)}
    header = {"schema": PORT_SNAPSHOT_SCHEMA, "rigor": "0.3.8", "vocabulary": 1, "config_digest": "d"}
    port_header = {**header, "rigor": "0.0.1"}

    def verdicts(ref, rs):
        return compare_snapshots(ref, rs)[0]

    round_trip = render_snapshot(header, {'A#a"q': row(["io"], ex=False, un=1,
                                                       de=["io.db"])}, {})
    checks = [
        ...
        ("a `schema:` bump fires",
         len(compare_snapshot_headers({**header, "schema": PORT_SNAPSHOT_SCHEMA + 1},
                                      port_header)) == 1),
